...
object DateUtil {
  ...
  //计算时间差 Days
  def dateTimeDiffInNow(beginDateTime: String, endDateTime: DateTime, datePattern: String = "yyyy-MM-dd HH:mm:ss"): Long = {
    try {
      math.abs(endDateTime.getMillis -
        DateTimeFormat.forPattern(datePattern).parseDateTime(beginDateTime).getMillis) / 86400000
    } catch {
      case _: Throwable => 0
    }
  ...
  def dateTimeDiffInDayWithoutAbs(beginDateTime: String, endDateTime: String, datePattern: String = "yyyy-MM-dd HH:mm:ss"): Long = {
    try {
      (DateTimeFormat.forPattern(datePattern).parseDateTime(endDateTime).getMillis -
        DateTimeFormat.forPattern(datePattern).parseDateTime(beginDateTime).getMillis) / 86400000
    } catch {
  ...
  */
  ...
  def is_inside_month_range(months: Int, startDate: String, endDate: String,
                            datePattern: String = "yyyy-MM-dd"): Boolean = {
    val period_month = new Period(DateTimeFormat.forPattern(datePattern).parseDateTime(startDate),
      DateTimeFormat.forPattern(datePattern).parseDateTime(endDate)).getMonths
    if (period_month < months) true else false
  }


//  判断时间是AM还是PM
  def getAMorPM(time:String, sdf: SimpleDateFormat) ={
    try {
